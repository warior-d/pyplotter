...
class Main(QWidget):
    ...
    def getCoordFromCentrPoint(self, x_ground, y_ground, x_current, y_current):
        ...
        need_point = geodesic(kilometers=lengh_meters / 1000).destination(Point(self.centrCoordsPointX, self.centrCoordsPointY),
                                                                          degs).format_decimal()
        print('getCoordFromCentrPoint', need_point.split(', '))
        # вывод - в координатах.
        return need_point

    def getCoordFromCentrPoint2(self, x_ground, y_ground, x_current, y_current):
        grid = int(Settings.GRID_SCALE[Settings.CURRENT_MASHTAB - 1])
        gridStep = Settings.GRID_STEP
        pixelLenght = grid / gridStep
        delta_x = x_current - x_ground
        ...
        need_point = geodesic(kilometers=lengh_meters / 1000).destination(Point(Settings.CENTR_LAT, Settings.CENTR_LON),
                                                                          degs).format_decimal()
        print('getCoordFromCentrPoint2', need_point.split(', '))
        # вывод - в координатах.
        return need_point
    ...
    def updateCentrPoint(self, newLat = 0, newLon = 0):
        ...
        if self.centrCoordsPointX == 0 and self.centrCoordsPointY == 0:
            ...
            print('initcoords', self.centrCoordsPointX, self.centrCoordsPointY)
        else:
    ...
    def getCentrPoint(self):
        return self.centrCoordsPointX, self.centrCoordsPointY

    def printCentrPoint(self):
        print('printCentrPoint', self.centrCoordsPointX, self.centrCoordsPointY)

    def doCentr(self):
        self.efemer_centr.setX(int(Settings.DESCTOP_WIDHT / 2))
        self.efemer_centr.setY(int(Settings.DESCTOP_HEIGHT / 2))
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            print('click! ', self.getCoordFromCentrPoint(event.pos().x(),
                                                         event.pos().y(),
                                                         int(Settings.DESCTOP_WIDHT / 2),
                                                         int(Settings.DESCTOP_HEIGHT / 2)))
            print('click2! ', self.getCoordFromCentrPoint2(event.pos().x(), event.pos().y(),
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self.updateCentrPoint(Settings.CENTR_LAT, Settings.CENTR_LON)
            self.printCentrPoint()
            self.doCentr()
    ...
    def mouseMoveEvent(self, event):
        ...
        new_pos_center = self.efemer_centr + delta
        newCentr = self.getCoordFromCentrPoint(new_pos_center.x(), new_pos_center.y(),
                                               int(Settings.DESCTOP_WIDHT / 2), int(Settings.DESCTOP_HEIGHT / 2))
        Settings.CENTR_LAT, Settings.CENTR_LON = newCentr.split(', ')
